...
-- =============================================
CREATE PROCEDURE [dbo].[SP_Nivel1_C2_Consultar]
...
BEGIN
	...
	set @QRYCOMMAND  = @QRYCOMMAND + ltrim(rtrim(@desc)) + ' AS DESCRIPCION, ';
	set @QRYCOMMAND  = @QRYCOMMAND + ltrim(rtrim(@desc2)) + ' AS DESCRIPCION2,CAST(ACTIVO AS INT) AS ACTIVO from ';
	...
	begin
		...
		set @QRYCOMMAND  = @QRYCOMMAND + ' or ' + UPPER(ltrim(rtrim(@desc2))) + ' LIKE ''%'' + UPPER(ltrim(rtrim(@descValor))) + ''%'')';
	end
